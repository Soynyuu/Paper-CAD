...
class GeometryAnalyzer:
    ...
    def __init__(self):
        ...
        # face index -> set of adjacent face indices (built from OCCT topology)
    ...
    def analyze_brep_topology(self, solid_shape):
        ...
        try:
            # --- 面（Face）の解析 ---
            # First pass: collect all TopoDS_Face objects and build index mapping
            face_explorer = TopExp_Explorer(solid_shape, TopAbs_FACE)
            face_index = 0
            topo_faces = []  # ordered list of TopoDS_Face

            while face_explorer.More():
                face = face_explorer.Current()
                topo_faces.append(topods.Face(face))
                face_explorer.Next()

            # Build OCCT edge->face adjacency map using topexp.MapShapesAndAncestors
            edge_face_map = TopTools_IndexedDataMapOfShapeListOfShape()
            topexp.MapShapesAndAncestors(
                solid_shape, TopAbs_EDGE, TopAbs_FACE, edge_face_map
            )

            # Build face-to-index lookup (IsSame-based, since TopoDS_Shape hash is identity-based)
            # We need to match TopoDS_Face objects from the map back to our indexed list
            face_to_index: Dict[int, int] = {}
            ...
            for edge_i in range(1, num_edges + 1):
                ...
                for i in range(len(sharing_indices)):
                    for j in range(i + 1, len(sharing_indices)):
                        fi, fj = sharing_indices[i], sharing_indices[j]
            ...
            self.stats["planar_faces"] = sum(
                1 for f in self.faces_data if f["surface_type"] == "plane"
            )
            self.stats["cylindrical_faces"] = sum(
                1 for f in self.faces_data if f["surface_type"] == "cylinder"
            )
            self.stats["conical_faces"] = sum(
                1 for f in self.faces_data if f["surface_type"] == "cone"
            )
            self.stats["other_faces"] = sum(
    ...
    def _extract_wire_points_parametric(
        self, wire, face_adaptor, num_points: int = 100
    ...
    def _sample_edge_points_parametric(
        self, curve_2d, face_adaptor, u_min, u_max, num_samples: int = 20
    ) -> List[Tuple[float, float, float]]:
        """
    ...
    def _sample_edge_points_3d(
        self, edge, num_samples: int = 20
    ) -> List[Tuple[float, float, float]]:
        """
    ...
    def _extract_wire_points_fallback(
        self, wire, num_points: int = 50
    ) -> List[Tuple[float, float, float]]:
        """
    ...
    def _analyze_general_surface(self, surface_adaptor):
        ...
        """
        # 一般的な曲面は平面として近似展開
        return {"unfold_method": "planar_approximation"}
