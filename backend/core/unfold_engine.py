...
class UnfoldEngine:
    ...
    def __init__(self, scale_factor: float = 10.0, tab_width: float = 0.0):
        ...
        self.adjacency_map: Dict[int, Set[int]] = defaultdict(set)
    ...
    def set_geometry_data(
        ...
        Args:
            ...
            adjacency_map: OCCT topology-based face adjacency map (face_idx -> set of adjacent face_idx)
        """
        self.faces_data = faces_data
        self.edges_data = edges_data
        self.adjacency_map = (
            adjacency_map if adjacency_map is not None else defaultdict(set)
        )

    def group_faces_for_unfolding(
        self,
        max_faces: int = 20,
        enable_grouping: bool = True,
        normal_threshold: float = 0.95,
        generate_unfolding_net: bool = False,
    ) -> List[List[int]]:
        """
        展開可能な面をグループ化。
        ...
        unfoldable_faces = [
            i for i, face in enumerate(self.faces_data) if face["unfoldable"]
        ]
        ...
        groups = []
        processed = set()
        unfoldable_set = set(unfoldable_faces)
        ...
        for start_face_idx in unfoldable_faces:
            ...
            while queue:
                ...
                # Only check topologically adjacent faces (O(1) lookup from adjacency map)
                # instead of iterating all unfoldable faces (O(F))
                for candidate_idx in self.adjacency_map.get(current_face_idx, ()):
    ...
    def _are_all_coplanar(
        self, face_indices: List[int], threshold: float = 0.95
    ) -> bool:
        """
        すべての面が同一平面かチェック。
    ...
    def _unfold_spanning_tree(self, group_idx: int, face_indices: List[int]) -> Dict:
        ...
        unfolded_faces[root_face_idx] = {
            "polygons": self._extract_face_2d_shape(
                root_face_idx, root_normal, root_origin
            ),
        ...
        # BFSで隣接する面を展開
        # Use deque for O(1) popleft; use adjacency_map for O(1) neighbor lookup
        face_indices_set = set(face_indices)
        queue = deque([root_face_idx])
        processed = {root_face_idx}

        while queue:
            current_face_idx = queue.popleft()

            # Only check topologically adjacent faces from adjacency_map
            for candidate_idx in self.adjacency_map.get(current_face_idx, ()):
                if candidate_idx in processed:
                    continue
                if candidate_idx not in face_indices_set:
                    continue
                ...
                # 候補面を展開
                candidate_polygons = self._extract_face_2d_shape(
                    candidate_idx, candidate_normal, candidate_origin
                )
    ...
    def _unfold_cylindrical_group(
        ...
        for face_idx in cylindrical_faces:
            ...
            # 円筒面の正確な2D形状を取得
            cylinder_polygons = self._extract_cylindrical_face_2d(
                face_idx, axis, center, radius
            )
    ...
    def _unfold_conical_group(self, group_idx: int, face_indices: List[int]) -> Dict:
        ...
        for face_idx in face_indices:
            ...
            # 円錐面の正確な2D形状を取得
            cone_polygons = self._extract_conical_face_2d(
                face_idx, apex, axis, radius, semi_angle
            )
            if cone_polygons:
    ...
    def _extract_face_2d_shape(
        self, face_idx: int, normal: np.ndarray, origin: np.ndarray
    ) -> List[List[Tuple[float, float]]]:
        """
        ...
        for boundary_idx, boundary in enumerate(face_data["boundary_curves"]):
            ...
            if len(boundary) >= 3:
                # 3D境界点を2D平面に正確に投影
                projected_boundary = self._project_points_to_plane_accurate(
                    boundary, normal, origin
                )

                # 境界線を単純化（正方形/長方形の場合は4点に削減）
                simplified_boundary = self._simplify_boundary_polygon(
                    projected_boundary
                )
    ...
    def _simplify_boundary_polygon(
    ...
    def _remove_duplicate_points_2d(
        self, points_2d: List[Tuple[float, float]], tolerance: float = 1e-6
    ) -> List[Tuple[float, float]]:
        """
        重複点を除去（2D点用）。
        ...
        for i in range(1, len(points_2d)):
            ...
            # 距離チェック
            distance = math.sqrt(
                (current[0] - last[0]) ** 2 + (current[1] - last[1]) ** 2
            )
    ...
    def _ensure_counterclockwise_order(
        self, points_2d: List[Tuple[float, float]]
    ) -> List[Tuple[float, float]]:
        """
        ...
        for i in range(n):
            j = (i + 1) % n
            signed_area += (points_2d[j][0] - points_2d[i][0]) * (
                points_2d[j][1] + points_2d[i][1]
            )
    ...
    def _extract_corners_by_angle(
        self, points_2d: List[Tuple[float, float]], num_corners: int
    ) -> List[Tuple[float, float]]:
        """
        角度変化を基に多角形の角を抽出。
    ...
    def _extract_convex_hull_corners(
        self, points_2d: List[Tuple[float, float]]
    ) -> List[Tuple[float, float]]:
        """
    ...
    def _thin_out_points(
        self, points_2d: List[Tuple[float, float]], max_points: int = 12
    ) -> List[Tuple[float, float]]:
        """
        点群を適度に間引く。
    ...
    def _simplify_rdp(
        self, points_2d: List[Tuple[float, float]], epsilon: float = 0.5
    ) -> List[Tuple[float, float]]:
        """
    ...
    def _rdp_recursive(
        self, points: List[Tuple[float, float]], epsilon: float
    ) -> List[Tuple[float, float]]:
        """
    ...
    def _extract_cylindrical_face_2d(
        self, face_idx: int, axis: np.ndarray, center: np.ndarray, radius: float
    ) -> List[List[Tuple[float, float]]]:
        """
        円筒面の正確な2D形状を抽出。
        ...
        for boundary in face_data["boundary_curves"]:
            if len(boundary) >= 3:
                # 3D境界点を円筒展開
                unfolded_boundary = self._unfold_cylindrical_points_accurate(
                    boundary, axis, center, radius
                )
    ...
    def _unfold_cylindrical_points_accurate(
        self,
        points_3d: List[Tuple[float, float, float]],
        axis: np.ndarray,
        center: np.ndarray,
    ...
    def _extract_conical_face_2d(
        ...
        for boundary in face_data["boundary_curves"]:
            if len(boundary) >= 3:
                # 3D境界点を円錐展開
                unfolded_boundary = self._unfold_conical_points_accurate(
                    boundary, apex, axis, radius, semi_angle
                )
    ...
    def _unfold_conical_points_accurate(
        self,
        points_3d: List[Tuple[float, float, float]],
        apex: np.ndarray,
        axis: np.ndarray,
            ...
            if distance > 1e-6:
                ...
                if abs(semi_angle) > 1e-6:
                    ...
                    if np.linalg.norm(radial_vec) > 1e-6:
                        ...
                        theta = math.atan2(
                            np.dot(radial_vec, np.cross(axis, ref_dir)),
    ...
    def _generate_cylindrical_tabs(
        self, cylindrical_faces: List[int], planar_faces: List[int]
    ) -> List[List[Tuple[float, float]]]:
        """
        円筒面グループ用のタブを生成。
    ...
    def _expand_face_group(
        self,
        current_group: List[int],
        used_faces: set,
        available_faces: List[int],
        ...
        for face_idx in available_faces:
            if (
                ...
                and face_idx not in current_group
                and self.faces_data[face_idx]["surface_type"]
                == last_face["surface_type"]
            ):
                # 隣接判定（簡易版 - 重心距離による）
                if self._are_faces_adjacent(last_face_idx, face_idx):
                    ...
                    if len(current_group) < max_group_size:
                        self._expand_face_group(
                            current_group, used_faces, available_faces, max_group_size
                        )
                    break
    ...
    def _are_faces_adjacent(
        ...
        OCCT topology-based adjacency_map がある場合は O(1) ルックアップ。
        フォールバック: 頂点の共有による O(V1*V2) 判定。

        Args:
            face_idx1: 面1のインデックス
            face_idx2: 面2のインデックス
            tolerance: 頂点マッチングの許容誤差（mm単位、フォールバック時のみ使用）

        Returns:
            bool: 隣接している場合True
        """
        # Fast path: use OCCT topology-based adjacency map
        if self.adjacency_map:
            return face_idx2 in self.adjacency_map.get(face_idx1, ())

        # Fallback: vertex-sharing based adjacency (O(V1*V2))
        face1 = self.faces_data[face_idx1]
        face2 = self.faces_data[face_idx2]
