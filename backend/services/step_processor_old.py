...
class StepUnfoldGenerator:
    ...
    def _simplify_boundary_polygon(
        ...
        if self._is_triangular_boundary(cleaned_points):
            ...
            logger.debug(
                f"境界線簡略化: {len(points_2d)}点 → {len(result)}点（三角形）"
            )
            return result
        ...
        if self._is_rectangular_boundary(cleaned_points):
            ...
            logger.debug(
                f"境界線簡略化: {len(points_2d)}点 → {len(result)}点（四角形）"
            )
            return result
        ...
        if self._is_pentagonal_boundary(cleaned_points):
            ...
            logger.debug(
                f"境界線簡略化: {len(points_2d)}点 → {len(result)}点（五角形）"
            )
            return result
        ...
        logger.debug(
            f"境界線簡略化: {len(points_2d)}点 → {len(result)}点（一般多角形）"
        )
        return result
    ...
    def export_to_svg(self, placed_groups: List[Dict], output_path: str) -> str:
        ...
        if overall_bbox["width"] > 0 and overall_bbox["height"] > 0:
            # 内容に応じてスケールファクターを調整
            content_scale = min(
                min_canvas_size / max(overall_bbox["width"], overall_bbox["height"]),
                10.0,
            )
        ...
        # SVGサイズ計算
        svg_width = max(
            min_canvas_size, overall_bbox["width"] * self.scale_factor + 2 * margin
        )
        svg_height = max(
            min_canvas_size,
            overall_bbox["height"] * self.scale_factor + 2 * margin + 120,
        )  # タイトル・スケール用
        ...
        content_offset_y = (
            margin - overall_bbox["min_y"] * self.scale_factor + 50
        )  # タイトル分下げる
        ...
        for group_idx, group in enumerate(placed_groups):
            ...
            for poly_idx, polygon in enumerate(group["polygons"]):
                if len(polygon) >= 3:
                    ...
                    points = [
                        (
                            x * self.scale_factor + content_offset_x,
                            y * self.scale_factor + content_offset_y,
                        )
                        for x, y in polygon
                    ]
    ...
    def _add_scale_bar(self, dwg, svg_width: float, svg_height: float, margin: float):
        ...
        dwg.add(
            dwg.line(
                start=(bar_x + bar_length_px, bar_y - 8),
        ...
        dwg.add(
            dwg.text(
                scale_text,
                insert=(bar_x + bar_length_px / 2, bar_y - 15),
                text_anchor="middle",
                class_="scale-text",
            )
        )

    def _add_technical_notes(
        self, dwg, svg_width: float, svg_height: float, margin: float
    ):
        """技術注記・凡例追加"""
        ...
        for i, note in enumerate(notes):
            dwg.add(
                dwg.text(note, insert=(notes_x, notes_y + i * 20), class_="note-text")
            )

    def generate_brep_papercraft(
        self, request, output_path: Optional[str] = None
    ) -> Tuple[str, Dict]:
        """
        BREPソリッドから展開図を一括生成。
        商用グレードの完全なワークフロー。
        ...
        try:
            ...
            self.stats["unfoldable_faces"] = sum(
                len(group["polygons"]) for group in placed_groups
            )
