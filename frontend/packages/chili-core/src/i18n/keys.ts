// Part of the Chili3d Project, under the AGPL-3.0 License.
// See LICENSE file in the project root for full license information.

const I18N_KEYS = [
    "arc.angle",
    "arc.start",
    "axis.x",
    "axis.y",
    "axis.z",
    "body.arc",
    "body.bolean",
    "body.box",
    "body.cylinder",
    "body.cone",
    "body.group",
    "body.sphere",
    "body.ellipse",
    "body.pyramid",
    "body.circle",
    "body.face",
    "body.fuse",
    "body.imported",
    "body.line",
    "body.multiShape",
    "body.polygon",
    "body.prism",
    "body.rect",
    "body.revol",
    "body.sweep",
    "body.wire",
    "body.editableShape",
    "body.meshNode",
    "box.dx",
    "box.dy",
    "box.dz",
    "circle.center",
    "circle.radius",
    "ellipsoid.radiusX",
    "ellipsoid.radiusY",
    "ellipsoid.radiusZ",
    "ellipse.majorRadius",
    "ellipse.minorRadius",
    "command.act.alignCamera",
    "command.boolean.common",
    "command.boolean.cut",
    "command.boolean.join",
    "command.convert.fuse",
    "command.convert.revol",
    "command.convert.sweep",
    "command.convert.toFace",
    "command.convert.toShell",
    "command.convert.toSolid",
    "command.convert.toWire",
    "command.convert.curveProjection",
    "command.create.arc",
    "command.create.bezier",
    "command.create.box",
    "command.create.circle",
    "command.create.cone",
    "command.create.copyShape",
    "command.create.cylinder",
    "command.create.ellipse",
    "command.create.ellipsoid",
    "command.create.extrude",
    "command.create.folder",
    "command.create.group",
    "command.create.line",
    "command.create.offset",
    "command.create.polygon",
    "command.create.pyramid",
    "command.create.rect",
    "command.create.section",
    "command.create.sphere",
    "command.create.thickSolid",
    "command.doc.new",
    "command.doc.open",
    "command.doc.save",
    "command.doc.saveToFile",
    "command.edit.redo",
    "command.edit.undo",
    "command.file.export",
    "command.file.import",
    "command.file.importCityGML",
    "command.file.importCityGMLByAddress",
    "command.file.importCityGMLByCesium",
    "command.file.importPlateauBuilding",
    "command.file.stepUnfold",
    "command.file.assemblyMode",
    "citygml.defaultHeight",
    "citygml.buildingLimit",
    "citygml.autoReproject",
    "citygml.buildingIds",
    "citygml.filterAttribute",
    "plateau.searchQuery",
    "plateau.searchRadius",
    "plateau.buildingLimit",
    "plateau.autoReproject",
    "plateau.mergeBuildingParts",
    "plateau.cesium.selectCity",
    "plateau.cesium.clickToSelect",
    "plateau.cesium.selectedCount",
    "plateau.cesium.importSelected",
    "plateau.cesium.clearSelection",
    "plateau.cesium.zoomToSelection",
    "plateau.cesium.instructions.click",
    "plateau.cesium.instructions.ctrlClick",
    "plateau.cesium.instructions.clearArea",
    "plateau.cesium.noSelection",
    "plateau.cesium.clickToSelectBuilding",
    "plateau.cesium.ctrlClickForMultiple",
    "plateau.cesium.loading",
    "error.plateau.selectAtLeastOne",
    "command.measure.angle",
    "command.measure.length",
    "command.measure.select",
    "command.modify.applyTexture",
    "command.modify.array",
    "command.modify.break",
    "command.modify.brushAdd",
    "command.modify.brushRemove",
    "command.modify.brushClear",
    "command.modify.chamfer",
    "command.modify.deleteNode",
    "command.modify.editTextureRotation",
    "command.modify.explode",
    "command.modify.fillet",
    "command.modify.mirror",
    "command.modify.move",
    "command.modify.removeFeature",
    "command.modify.removeShapes",
    "command.modify.rotate",
    "command.modify.split",
    "command.modify.trim",
    "command.special.last",
    "command.test.performace",
    "command.settings.keymap",
    "command.settings.units",
    "settings.keymap",
    "settings.keymap.preset",
    "settings.keymap.shortcuts",
    "settings.keymap.search",
    "settings.keymap.export",
    "settings.keymap.import",
    "settings.keymap.reset",
    "settings.keymap.noResults",
    "settings.keymap.resetConfirm",
    "texture.selection.title",
    "texture.pattern",
    "texture.preview",
    "texture.tileCount",
    "command.wechat.group",
    "command.workingPlane.alignToPlane",
    "command.workingPlane.fromSection",
    "command.workingPlane.set",
    "command.workingPlane.toggleDynamic",
    "common.angle",
    "common.area",
    "common.back",
    "common.language",
    "common.3DNavigation",
    "common.cancel",
    "common.clone",
    "common.color",
    "common.confirm",
    "common.general",
    "common.length",
    "common.dir",
    "common.material",
    "common.matrix",
    "common.name",
    "common.normal",
    "common.opacity",
    "common.thickness",
    "common.type",
    "common.volume",
    "dialog.title.selectWorkingPlane",
    "entity.editable",
    "entity.parameter",
    "error.default:{0}",
    "error.input.cannotInputANumber",
    "error.input.invalidNumber",
    "error.input.threeNumberCanBeInput",
    "error.input.unsupportedInputs",
    "error.import.unsupportedFileType:{0}",
    "error.import.notCityGML",
    "error.import.invalidFileExtension",
    "error.citygml.conversionFailed:{0}",
    "error.citygml.importFailed:{0}",
    "error.plateau.emptyQuery",
    "error.plateau.emptyMeshCode",
    "error.plateau.searchFailed:{0}",
    "error.plateau.noBuildingsFound:{0}",
    "error.plateau.conversionFailed:{0}",
    "error.plateau.importFailed:{0}",
    "error.plateau.notABuilding",
    "error.plateau.noGmlId",
    "error.plateau.cityNotFound:{0}",
    "error.plateau.cityLoadFailed:{0}:{1}",
    "error.plateau.selectionFailed:{0}",
    "error.plateau.cesiumInitFailed:{0}",
    "error.export.noNodeCanBeExported",
    "error.textureService.notFound",
    "error.faceNumber.notFound",
    "error.texture.notApplied",
    "error.geometryNode.notFound",
    "file.format",
    "home.recent",
    "home.welcome",
    "home.createFromAddress",
    "items.header",
    "items.tool.delete",
    "items.tool.expandAll",
    "items.tool.newFolder",
    "items.tool.unexpandAll",
    "line.end",
    "line.start",
    "line.type.line",
    "line.type.xline",
    "material.texture.image",
    "material.texture.rotation",
    "material.texture.wrapS",
    "material.texture.wrapT",
    "material.texture.repeat",
    "material.texture.offset",
    "material.map",
    "material.specular",
    "material.shininess",
    "material.emissive",
    "material.specularMap",
    "material.normalMap",
    "material.bumpMap",
    "material.roughnessMap",
    "material.emissiveMap",
    "material.metalness",
    "material.metalnessMap",
    "material.roughness",
    "model.visible",
    "prompt.pickCircleCenter",
    "prompt.pickFistPoint",
    "prompt.pickNextPoint",
    "prompt.pickRadius",
    "option.command.repeat",
    "option.command.isFace",
    "option.command.isRoundCorner",
    "option.command.thickness",
    "option.command.isConnected",
    "option.command.isConvertInstance",
    "option.command.insertPoint",
    "polygon.points",
    "prompt.default{0}{1}",
    "prompt.deleteDocument{0}",
    "prompt.polygon.close",
    "prompt.saveDocument{0}",
    "prompt.select.edges",
    "prompt.select.faces",
    "prompt.select.models",
    "prompt.select.noModelSelected",
    "prompt.select.section",
    "prompt.select.shape",
    "prompt.select.solids",
    "prompt.select.path",
    "prompt.select.vertexes",
    "prompt.select.wires",
    "prompt.selectFacesForTexture",
    "prompt.selectTexturedFace",
    "properties.group.transform",
    "properties.header",
    "properties.multivalue",
    "rect.dx",
    "rect.dy",
    "ribbon.group.2d",
    "ribbon.group.3d",
    "ribbon.group.act",
    "ribbon.group.boolean",
    "ribbon.group.brush",
    "ribbon.group.converter",
    "ribbon.group.draw",
    "ribbon.group.edit",
    "ribbon.group.importExport",
    "ribbon.group.measure",
    "ribbon.group.modify",
    "ribbon.group.other",
    "ribbon.group.selection",
    "ribbon.group.tools",
    "ribbon.group.transform",
    "ribbon.group.workingPlane",
    "ribbon.tab.converter",
    "ribbon.tab.draw",
    "ribbon.tab.file",
    "ribbon.tab.modify",
    "ribbon.tab.tools",
    "ribbon.tab.startup",
    "snap.center",
    "snap.end",
    "snap.intersection",
    "snap.mid",
    "snap.perpendicular",
    "snap.nearest",
    "statusBar.snap",
    "statusBar.tracking",
    "texture.patternId",
    "toast.command.{0}excuting",
    "toast.converter.error",
    "toast.converter.invalidColor",
    "toast.delete{0}Objects",
    "toast.document.noActivated",
    "toast.document.saved",
    "toast.downloading",
    "toast.excuting{0}",
    "toast.fail",
    "toast.read.error",
    "toast.select.noSelected",
    "toast.success",
    "toast.citygml.converting",
    "toast.citygml.importSuccess",
    "toast.plateau.searching",
    "toast.plateau.converting:{0}",
    "toast.plateau.importSuccess:{0}:{1}m",
    "toast.plateau.allConversionsFailed:{0}",
    "toast.plateau.cesiumImportSuccess:{0}",
    "toast.plateau.cesiumImportSuccessWithFailures:{0}:{1}:{2}",
    "toast.plateau.cesiumImportFailed:{0}",
    "toast.plateau.importSuccess:{0}",
    "toast.plateau.importSuccessWithFailures:{0}:{1}:{2}",
    "toast.plateau.importFailed:{0}",
    "toast.plateau.tilesetLoadFailed:{0}",
    "toast.stepUnfold.success",
    "toast.stepUnfold.error",
    "stepUnfold.scale",
    "stepUnfold.scale.label",
    "stepUnfold.modelSize",
    "stepUnfold.experimental",
    "stepUnfold.experimentalWarning",
    "stepUnfold.layoutMode",
    "stepUnfold.layoutMode.canvas",
    "stepUnfold.layoutMode.paged",
    "stepUnfold.pageFormat",
    "stepUnfold.pageFormat.A4",
    "stepUnfold.pageFormat.A3",
    "stepUnfold.pageFormat.Letter",
    "stepUnfold.pageOrientation",
    "stepUnfold.pageOrientation.portrait",
    "stepUnfold.pageOrientation.landscape",
    "assembly.title",
    "assembly.close",
    "assembly.helpText",
    "assembly.3dModel",
    "assembly.2dUnfold",
    "assembly.status",
    "assembly.ready",
    "assembly.selectedFace",
    "assembly.faceCount",
    "assembly.faceNumberInput",
    "assembly.enterFaceNumber",
    "assembly.highlight",
    "assembly.clearSelection",
    "assembly.reset3DView",
    "assembly.reset2DView",
    "assembly.zoomIn",
    "assembly.zoomOut",
    "assembly.loadingModel",
    "assembly.loadingUnfold",
    "assembly.faceSelected:{0}",
    "assembly.shortcutHint",
    "toast.assemblyMode.started",
    "toast.assemblyMode.error",
    "toast.assemblyMode.unfoldError",
    "toast.assemblyMode.closed",
    "toast.assemblyMode.invalidFaceNumber",
    "toast.assemblyMode.faceNotFound:{0}",
    "toast.texture.rotationSet:{0}",
    "toast.snap.notFoundValidPoint",
    "transform.rotation",
    "transform.scale",
    "transform.translation",
    "vertex.point",
    "unit.millimeter",
    "unit.centimeter",
    "unit.meter",
    "unit.inch",
    "unit.foot",
    "unit.degree",
    "unit.radian",
    "unit.lengthUnit",
    "unit.angleUnit",
    "unit.lengthPrecision",
    "unit.anglePrecision",
    "unit.settings",
] as const;

export type I18nKeys = (typeof I18N_KEYS)[number];
